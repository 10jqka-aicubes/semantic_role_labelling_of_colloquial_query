...
class BLSTM_CRF(object):
    ...
    def add_blstm_crf_layer(self, crf_only):
        ...
        if self.labels is not None:
            ...
            loss, pred_ids = None, None
            trans = self.crf_layer_trans(logits)
    ...
    def crf_layer(self, logits):
        ...
        with tf.variable_scope("crf_loss"):
            ...
            log_likelihood, trans = tf.contrib.crf.crf_log_likelihood(
                inputs=logits, tag_indices=self.labels, transition_params=trans, sequence_lengths=self.lengths
            )
            return tf.reduce_mean(-log_likelihood), trans

    def crf_layer_trans(self, logits):
        with tf.variable_scope("crf_loss"):
            trans = tf.get_variable(
                "transitions",
                shape=[self.num_labels, self.num_labels],
