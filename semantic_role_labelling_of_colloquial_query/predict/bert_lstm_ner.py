...
flags.DEFINE_string(
    "data_dir",
    os.path.join(root_path, "NERdata"),
    "The input datadir.",
)
...
flags.DEFINE_string(
    "output_dir", os.path.join(root_path, "output"), "The output directory where the model checkpoints will be written."
)

## Other parameters
flags.DEFINE_string(
    "init_checkpoint",
    os.path.join(bert_path, "bert_model.ckpt"),
    "Initial checkpoint (usually from a pre-trained BERT model).",
)

flags.DEFINE_bool("do_lower_case", True, "Whether to lower case the input text.")

flags.DEFINE_integer("max_seq_length", 40, "The maximum total input sequence length after WordPiece tokenization.")
...
flags.DEFINE_string(
    "vocab_file", os.path.join(bert_path, "vocab.txt"), "The vocabulary file that the BERT model was trained on."
)

tf.flags.DEFINE_string("master", None, "[Optional] TensorFlow master URL.")
flags.DEFINE_integer("num_tpu_cores", 8, "Only used if `use_tpu` is True. Total number of TPU cores to use.")
flags.DEFINE_string(
    "data_config_path", os.path.join(root_path, "data.conf"), "data config file, which save train and dev config"
)
# lstm parame
...
flags.DEFINE_string("predict_result_dir", "", "predict result file dir")

# 移除模型中的Adam相关参数，使得最终模型文件为300-400M， 不会是原来的1.2G， 移除后的模型可以用于预测阶段。
# Add code to remove the adam related parameters in the model, and reduce the size of the model file
# from 1.3GB to 400MB.
        ...
        with codecs.open(input_file, "r", encoding="utf-8") as f:
            ...
            for line in f:
                ...
                if len(tokens) == 2:
                    ...
                    if len(contends) == 0:
                        ll = " ".join([label for label in labels if len(label) > 0])
...
class NerProcessor(DataProcessor):
    def get_train_examples(self, data_dir):
        return self._create_example(self._read_data(os.path.join(data_dir, "train_res")), "train")

    def get_dev_examples(self, data_dir):
        return self._create_example(self._read_data(os.path.join(data_dir, "test_res")), "dev")

    def get_test_examples(self, data_dir):
        return self._create_example(self._read_data(os.path.join(data_dir, "test_res")), "test")
...
def model_fn_builder(
    ...
    def model_fn(features, labels, mode, params):
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            output_spec = tf.contrib.tpu.TPUEstimatorSpec(mode=mode, predictions=pred_ids, scaffold_fn=scaffold_fn)
        return output_spec
...
def main(_):
    ...
    run_config = tf.contrib.tpu.RunConfig(
        ...
        tpu_config=tf.contrib.tpu.TPUConfig(
            iterations_per_loop=FLAGS.iterations_per_loop, num_shards=FLAGS.num_tpu_cores
        ),
    )
    ...
    if FLAGS.do_predict:
        ...
        os.system("mkdir -p {}".format(FLAGS.predict_result_dir))
        output_predict_file = os.path.join(FLAGS.predict_result_dir, "predict.txt")
